...
func main() {
	...
	http.HandleFunc("/background", func(w http.ResponseWriter, req *http.Request) {
		...
		io.WriteString(w, "background transaction 1")
		time.Sleep(150 * time.Millisecond)
	})
	http.HandleFunc(newrelic.WrapHandleFunc(app, "/2016-08-15/proxy/test-apm/newrelic-golang-http/mysql", mysql))
	http.HandleFunc(newrelic.WrapHandleFunc(app, "/2016-08-15/proxy/test-apm/newrelic-golang-http/external", external))
	http.HandleFunc(newrelic.WrapHandleFunc(app, "/2016-08-15/proxy/test-apm/newrelic-golang-http/customEvent", customEvent))
	http.HandleFunc(newrelic.WrapHandleFunc(app, "/2016-08-15/proxy/test-apm/newrelic-golang-http/segments", segments))
	http.HandleFunc(newrelic.WrapHandleFunc(app, "/2016-08-15/proxy/test-apm/newrelic-golang-http/sleep", sleep))
